--[[
HI ASPECT AND COLB (Most likely lolmanurfunny too)
How was your day?

Ban command no longer works well, neither do homerun, heaven, hotwheels, etc.
]]
...
CMDs[#CMDs + 1] = {NAME = 'nohats / deletehats / rhats / makeawish [plr]', DESC = 'Deletes your hats'}
...
addcmd('rcar',{},function(args, speaker)
	...
	vehicleModel:MoveTo(Vector3.new(vehicleModel.PrimaryPart.Position.X, workspace.FallenPartsDestroyHeight+10, vehicleModel.PrimaryPart.Position.Z))
	notify('Successfully deleted '..vehicleModel.Properties.Owner.Value.Name..vehicleModel.Name.." ($"..itemListModule[vehicleModel.Name].Price..")")
end)
...
addcmd('getcar',{},function(args, speaker)
    ...
    for _,car in next, workspace.PlayerVehicles:GetChildren() do
        if car:FindFirstChild('VehicleSeat') and not car:FindFirstChild('VehicleSeat'):FindFirstChild('SeatWeld') and not found then
            ...
            if car.Body:FindFirstChildOfClass('Part').Material == Enum.Material.CorrodedMetal then return end
            execCmd('unlockcars')
            repeat wait(.15)
...
addcmd('avatar',{},function(args, speaker)
    local shirts
    local pants
    local hats
    local hairs
...
addcmd('speedometer',{'hud'},function(args, speaker)
    ...
    game:GetService("Players").LocalPlayer.Character.Humanoid.SeatPart.HeadsUpDisplay = false
    game:GetService("Players").LocalPlayer.Character.Humanoid.SeatPart.HeadsUpDisplay = true
end)
...
local bodyfuck=Instance.new("BodyThrust", game:GetService("Players").LocalPlayer.Character.Humanoid.SeatPart.Parent:FindFirstChild("Engine")) --nigga
for i = 1,3 do
bodyfuck.force=Vector3.new(0,-1000000,0)
end
wait()
bodyfuck:Destroy()
end)
		...
		if Players[v] == speaker then
		    ...
		    for _,hat in next, Players[v].Character:GetChildren() do
		        if hat:IsA('Accessory') then
    		        ...
    		        for i = 1,3 do
        		        speaker.Character.HumanoidRootPart.CFrame = Players[v].Character.HumanoidRootPart.CFrame
...
function aaa(speaker,target)
	if tools(speaker) then
	    execCmd('bypass')
	    for i, v in next, getconnections(game:GetService("Players").LocalPlayer.Character.DescendantRemoving) do
            v:Disable()
    	end
    	for i, v in next, getconnections(game:GetService("Players").LocalPlayer.Character.ChildRemoved) do
            v:Disable()
    	end
		local char = speaker.Character
...
addcmd('eat',{},function(args, speaker)
    ...
    if done == false then
        for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
            if v:IsA("Tool") and not done then
...
addcmd('ban',{'ab','attemptban'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
	    if tools(speaker) then
    		...
    		wait(1)
    		for _ = 1,10 do
        		hrp.CFrame = CFrame.new(workspace.Entities:FindFirstChild('ToolModel').hitbox.Position + Vector3.new(0,3,0))
        		wait(.1)
    		end
    		wait(.1)
    		execCmd('respawn')
...
addcmd('bandb',{},function(args, speaker)
	speaker.Character.HumanoidRootPart.CFrame = CFrame.new(workspace.Entities:FindFirstChild('ToolModel').hitbox.Position + Vector3.new(0,3,0))
end)
...
for i, v in next, workspace.BankBuilding:GetChildren() do
    if v.Name == 'Part' and v.ClassName == 'Part' then
       v:Destroy()
    end
end

workspace.BankBuilding.ChildAdded:Connect(function(child)
    game:GetService('RunService').RenderStepped:Wait()
    if child.Name == 'Part' and child.ClassName == 'Part' then
        child:Destroy() 
    end
end)